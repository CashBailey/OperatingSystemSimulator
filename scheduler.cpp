...
// ---------------------- FCFS (non-preemptive, matches provided logic) ----------------------
void run_fcfs(const vector<Process>& processes_in)
{
	...
	// 2) Run to completion in arrival order, computing the same metrics/prints as the snippet
	int currentTime = 0;
	...
	for (auto &p : processes)
	{
		...
		// Update stored accounting (useful for any follow-on analysis)
		p.remaining_time   = 0;
		...
		// Print exactly the snippet's table view (pid shown as "P<id>")
		cout << "P" << p.pid << "\t"
		     << p.arrival_time << "\t"
		     << p.Cpu_burst << "\t"
		     << start << "\t"
...
// ---------------------- Non-preemptive SJF (arrival-aware, existing) ----------------------
